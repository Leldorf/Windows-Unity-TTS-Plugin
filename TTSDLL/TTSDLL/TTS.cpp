...
BOOL TTS::SetVoice(const wchar_t* voiceName)
{
    ...
    this->SetErrorMessage("Exception: Not found voice name");
    return FALSE;
}
...
BOOL TTS::SetVoice(const int voiceNum)
{
    ...
    {
        this->SetErrorMessage("Exception: Out of voices index range");
        return FALSE;
    }
}
    ...
    for_each_voice([=, &isSuccess, &counter, &num, &out, &text, &path](auto voice, auto token, int count)
        {
            ...
            {
                this->SetErrorMessage("Cannot call ISpVoice::SetVoice");
                return false;
            }
            ...
            if (out && wcslen(path) != 0)
            {
                ...
                {
                    this->SetErrorMessage("Cannot call CoCreateInstance(CLSID_SpFileStream)");
                    return false;
                }

                if (FAILED(outputStream->Open(outputPath, SSFMCreateForWrite)))
                {
                    this->SetErrorMessage("Cannot open the output file");
                    return false;
                }

                if (FAILED(voice->SetOutput(outputStream, true)))
                {
                    this->SetErrorMessage("Cannot call ISpVoice::SetOutput");
                    return false;
                }
            ...
            {
                this->SetErrorMessage("Cannot call ISpVoice::Speak");
                return false;
            }
